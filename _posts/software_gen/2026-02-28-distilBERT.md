...
4. Task-Specific Fine-tuned Models - While the above are base models, DistilBERT is frequently fine-tuned for specific tasks via the Hugging Face library.
...
# Architecture
![alt text](/swe_assistant/docs/assets/distilbert.jpg "DistilBERT")

Drawing parallels with the original transformer architecture from the blog in the 'Introduction' section to the architecture above, we can see that the word embeddings are expected to output a 768 dim vector in the first layer. Following which we have a neat trick of the positional encoding added to the embeddings to give context on order of the input tokens. The next layer seems to be layer normalization. We have seen BatchNornm while training other deep learning architectures but in transformers, we use layer norm to normalize within the inputs passed. This is to prevent exploding/vanishing gradients and this is likely in NLP because we are working with sequential data of varying lengths. This is followed by a 10% dropout.
The next is the encoder block where we have 6 heads of the Query, Key, Value weight matrices which are square matrices of size 768 which is the embedding dimension. Th Q, K, V weight matrices are where self attention is captured which is the essence of the Transformers architecture. The square nature is not a fixed practice across all models. It just happens to be square in this model. This is again followed by a 10% dropout to avoid overfitting followed by a Layer Normalization.
The next block is the feed forward block with 2 layers. First layer has input dim of 768, blows it up to 3072 followed by next layer that brings it back down to 768. The activation function used is GELUActivation. One of the big problems of ReLU was the vanishing gradient problem as it has constant value of zero for all negative values. This lead to "dying" neurons. GeLU provides a chance for neurons to become active learner again as it has a soft gating unlike the hard gating of ReLU at 0.
Finally, it is the head that we have added to distilBERT for it to be able to do Binary Classification. So, the last block consists of 2 layers with first one having input dim of 768 and output dim of 768. Last layer brings it down to output dim of 2 followed by a 20% dropout.
